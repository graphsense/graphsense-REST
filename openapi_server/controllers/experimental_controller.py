...
import gsrest.service.experimental_service as service
...
async def get_tag_summary_by_entity(request: web.Request, currency, entity) -> web.Response:
    ...
    try:
        if 'currency' in ['','currency','entity']:
            if currency is not None:
                currency = currency.lower() 
        result = service.get_tag_summary_by_entity(request
