...
def from_row(currency,
    ...
    return TxUtxo(currency=currency,
                  ...
                  inputs=io_from_rows(currency, row, 'inputs', rates,
                                      include_io, include_nonstandard_io,
                                      include_io_index),
...
async def get_trace_txs(request, currency, tx, trace_index=None):
    ...
    if result and result["tx_hash"] == tx["tx_hash"]:
        ...
        return from_row(currency, result,
    ...
    if token_tx_id is not None:
        if is_eth_like(currency):
            ...
            if len(results):
                ...
                raise TransactionNotFoundException(currency, tx_ident,
                                                   token_tx_id)
        else:
...
async def get_tx_io(request, currency, tx_hash, io, include_nonstandard_io,
                    include_io_index):
    if is_eth_like(currency):
        raise NotFoundException('get_tx_io not implemented for ETH')
    result = await get_tx(request,
                          currency,
                          tx_hash,
                          include_io=True,
