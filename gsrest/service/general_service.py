...
async def get_statistics(request):
    ...
    return Stats(currencies=currency_stats, version=version, request_timestamp=tstamp)
...
async def search(request, q, currency=None, limit=10):
    ...
    result.actors = [LabeledItemRef(id=x.id, label=x.label) for x in r2.actor_labels]
