...
import gsrest.service.tags_service as tagsDAO
...
class LabelTags(Resource):
    ...
    def get(self):
        ...
        if currency:
            result = tagsDAO.list_tags(label, currency)
        else:
            result = []
            for currency in current_app.config['MAPPING']:
                if currency != "tagpacks":
                    tags = tagsDAO.list_tags(label, currency)
                    if tags:
                        result += tags
        if result:
            return result
...
class Taxonomies(Resource):
    ...
    def get(self):
        ...
        """
        return tagsDAO.list_taxonomies()
...
class TaxonomiesConcepts(Resource):
    ...
    def get(self, taxonomy):
        ...
        """
        return tagsDAO.list_concepts(taxonomy)
