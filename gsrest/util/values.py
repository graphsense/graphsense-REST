...
def make_values(value, eur, usd):
    return Values(
        value=value,
        fiat_values=[Rate('eur', round(eur, 2)),
...
def map_rates_for_peged_tokens(rates, token_config):
    ...
    if peg == "usd":
        if len(rates) != 2:
            raise Exception("Rates structure is expected "
                            f"to be a list of length 2: {rates}")
        ...
        raise Exception(
            "Currently only tokens pegged to ether or usd are supported")
...
def convert_token_values_map(currency, value_map, rates, token_configs):
    if value_map is None:
        ...
        return {
            token_currency.lower():
            convert_token_value(value, rates, token_configs[token_currency])
            for token_currency, value in value_map.items()
        }


def convert_value_impl(value, rates, factor):
    return Values(value=catchNaN(value),
                  fiat_values=[
                      Rate(r['code'],
...
def convert_token_value(value, rates, token_config):
    return convert_value_impl(value,
