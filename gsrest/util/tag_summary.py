...
async def get_tag_summary(get_tags_page_fn,
    ...
    return TagSummary(
        ...
        label_words_tag_cloud={
            word: TagCloudEntry(cnt=cnt, weighted=cnt / total_words)
            for word, cnt in label_word_counter.most_common(
                label_words_max_items)
        },
        label_tag_cloud={
            word: TagCloudEntry(cnt=cnt, weighted=cnt / tags_count)
            for word, cnt in full_label_counter.most_common(label_max_items)
        },
        concept_tag_cloud={
            word: TagCloudEntry(cnt=cnt, weighted=cnt / tags_count)
            for word, cnt in concepts_counter.most_common()
        },
        actors_tag_cloud={
            word: TagCloudEntry(cnt=cnt, weighted=cnt / tags_count)
            for word, cnt in actor_counter.most_common()
        })
