...
tag1 = AddressTag(category="organization",
...
tag3 = AddressTag(category="organization",
...
eth_tag1 = AddressTag(category=None,
                      label="TagA",
                      abuse=None,
                      lastmod=1,
                      source="sourceX",
...
eth_tag2 = AddressTag(category=None,
                      label="TagB",
                      abuse=None,
                      lastmod=1,
                      source="sourceY",
...
etag1 = AddressTag(category=tag1.category,
                   label=tag1.label,
                   abuse=tag1.abuse,
...
etag2 = AddressTag(tagpack_uri="https://tagpack_uri",
                   lastmod=1,
                   label="isolinks",
                   source="Unspecified",
                   category='exchange',
...
tag5 = AddressTag(tagpack_uri="https://tagpack_uri",
                  lastmod=1,
                  label="isolinks",
                  source="Unspecified",
                  category='exchange',
...
tag8 = AddressTag(category="organization",
                  label="labelX",
                  abuse=None,
                  lastmod=1560290400,
                  source="https://source",
...
etag4 = AddressTag(category=tag2.category,
                   label=tag2.label,
                   abuse=tag2.abuse,
...
async def list_address_tags(test_case):
    ...
    t2['entity'] = 456
    test_case.assertEqual([t1, t2], result['address_tags'])

    result = await test_case.request(path,
                                     auth='unauthorized',
                                     label='isolinks')
    test_case.assertEqual([t1], result['address_tags'])
    ...
    test_case.assertEqual([t2], result['address_tags'])
    result = await test_case.request(path_with_page,
                                     label='isolinks',
                                     pagesize=1,
                                     page=result['next_page'])
    ...
    test_case.assertEqual([eth_tag3.to_dict()], result['address_tags'])


organization = Concept(uri="https://organization",
                       id="organization",
                       taxonomy="entity",
                       label="An organization",
                       description="An organization is foo.")

exchange = Concept(uri="https://exchange",
                   id="exchange",
                   taxonomy="entity",
                   label="An exchange",
                   description="An exchange is foo.")

conceptB = Concept(uri="https://conceptB",
                   id="conceptB",
                   taxonomy="abuse",
                   label="Concept B",
